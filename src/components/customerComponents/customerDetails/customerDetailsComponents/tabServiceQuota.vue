<template>
	...
	<!-- 服务名额 -->
	<service-quota ref="serviceQuota" v-if="activeTab === 'serviceQuota'" />
</template>

<script>
import { computed, reactive, toRefs, ref, nextTick } from "vue";
import { useUserStore } from "@/store/modules/user";
import serviceQuota from "@/components/customerComponents/customerDetails/customerDetailsComponents/tabServiceQuota/serviceQuota.vue";
export default {
	name: "tabServiceQuota",
	components: { serviceQuota },
	...
	setup(props, { emit }) {
		...
		const customerTabsItemInit = item => {
			...
			switch (dataMap.activeTab) {
				case "serviceQuota":
					nextTick(() => {
						serviceQuota.value.initComponents(dataMap.params);
					});
					break;
			}
		};
